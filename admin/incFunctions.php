	...
	*/
		...
		$all_tables = [
			/* ['table_name' => [table props assoc array] */   
				'servers' => [
					'Caption' => 'Servers',
					'Description' => '',
					'tableIcon' => 'resources/table_icons/server.png',
					'group' => $tg[0],
					'homepageShowCount' => 1
				],
				'operating_systems' => [
					...
					'Description' => '',
					'tableIcon' => 'resources/table_icons/download_for_linux.png',
					'group' => $tg[1],
					'homepageShowCount' => 1
				],
				'hosting_providers' => [
					...
					'Description' => '',
					'tableIcon' => 'resources/table_icons/hostname.png',
					'group' => $tg[1],
					'homepageShowCount' => 1
				],
				'data_centers' => [
					...
					'Description' => '',
					'tableIcon' => 'resources/table_icons/server_stanchion.png',
					'group' => $tg[1],
					'homepageShowCount' => 1
				],
				'domains' => [
				...
				'dns_managers' => [
					'Caption' => 'DNS managers',
					'Description' => '',
					'tableIcon' => 'resources/table_icons/dns_setting.png',
					'group' => $tg[1],
					'homepageShowCount' => 1
				],
				'mail_providers' => [
					'Caption' => 'Mail providers',
					'Description' => '',
					'tableIcon' => 'resources/table_icons/mail_server_setting.png',
					'group' => $tg[1],
					'homepageShowCount' => 1
				],
				'clients' => [
				...
				'client_contacts' => [
					...
					'Description' => '',
					'tableIcon' => 'resources/table_icons/user_comment.png',
					'group' => $tg[0],
					'homepageShowCount' => 1
				],
				'server_events' => [
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = [
			...
			'servers' => ['Servers', '', 'resources/table_icons/server.png', 'Operations'],
			'operating_systems' => ['Operating systems', '', 'resources/table_icons/download_for_linux.png', 'Providers'],
			'hosting_providers' => ['Hosting providers', '', 'resources/table_icons/hostname.png', 'Providers'],
			'data_centers' => ['Data centers', '', 'resources/table_icons/server_stanchion.png', 'Providers'],
			'domains' => ['Domains', '', 'resources/table_icons/domain_template.png', 'Operations'],
			'subdomains' => ['Subdomains', '', 'resources/table_icons/domain_template.png', 'Operations'],
			'registrars' => ['Registrars', '', 'resources/table_icons/book_spelling.png', 'Providers'],
			'dns_managers' => ['DNS managers', '', 'resources/table_icons/dns_setting.png', 'Providers'],
			'mail_providers' => ['Mail providers', '', 'resources/table_icons/mail_server_setting.png', 'Providers'],
			'clients' => ['Clients', '', 'resources/table_icons/client_account_template.png', 'Operations'],
			'client_contacts' => ['Client contacts', '', 'resources/table_icons/user_comment.png', 'Operations'],
			'server_events' => ['Server events', '', 'resources/table_icons/file_extension_log.png', 'Events'],
			'domain_events' => ['Domain events', '', 'resources/table_icons/file_extension_log.png', 'Events'],
			'subdomain_events' => ['Subdomain events', '', 'resources/table_icons/file_extension_log.png', 'Events'],
			'client_events' => ['Client events', '', 'resources/table_icons/file_extension_log.png', 'Events'],
		];
	...
	function get_table_fields($tn = null) {
		...
		if($schema === null) {
			...
			$schema = [
				...
				'domains' => [
					'domain_name' => [
						'appgini' => "VARCHAR(100) NOT NULL PRIMARY KEY",
						'info' => [
							'caption' => 'Domain name',
							'description' => '',
						],
	...
	*/
	...
	function get_parent_tables($table) {
		...
		$parents = [
			'servers' => [
				'clients' => ['client'],
				'hosting_providers' => ['hosted_at'],
				'data_centers' => ['data_center'],
				'operating_systems' => ['os'],
			],
			'data_centers' => [
				'hosting_providers' => ['hosting_provider'],
			],
			'domains' => [
				'clients' => ['client'],
				'registrars' => ['registrar'],
				'dns_managers' => ['dns_manager'],
				'mail_providers' => ['mail_provider'],
			],
			'subdomains' => [
				'domains' => ['domain'],
				'servers' => ['server'],
	...
	function lookupQuery($tn, $lookupField) {
		...
		*/
		$lookupQuery = [
			'servers' => [
				'client' => 'SELECT `clients`.`id`, `clients`.`client` FROM `clients` ORDER BY 2',
				'hosted_at' => 'SELECT `hosting_providers`.`id`, `hosting_providers`.`hosting_provider` FROM `hosting_providers` ORDER BY 2',
				'data_center' => 'SELECT `data_centers`.`id`, `data_centers`.`data_center` FROM `data_centers` LEFT JOIN `hosting_providers` as hosting_providers1 ON `hosting_providers1`.`id`=`data_centers`.`hosting_provider` ORDER BY 2',
				'os' => 'SELECT `operating_systems`.`id`, `operating_systems`.`operating_system` FROM `operating_systems` ORDER BY 2',
			],
			'operating_systems' => [
			],
			'hosting_providers' => [
			...
			'data_centers' => [
				'hosting_provider' => 'SELECT `hosting_providers`.`id`, `hosting_providers`.`hosting_provider` FROM `hosting_providers` ORDER BY 2',
			],
			'domains' => [
				'client' => 'SELECT `clients`.`id`, `clients`.`client` FROM `clients` ORDER BY 2',
				'registrar' => 'SELECT `registrars`.`id`, `registrars`.`registrar` FROM `registrars` ORDER BY 2',
				'dns_manager' => 'SELECT `dns_managers`.`id`, `dns_managers`.`dns_manager` FROM `dns_managers` ORDER BY 2',
				'mail_provider' => 'SELECT `mail_providers`.`id`, `mail_providers`.`mail_provider` FROM `mail_providers` ORDER BY 2',
			],
			'subdomains' => [
				'domain' => 'SELECT `domains`.`domain_name`, `domains`.`domain_name` FROM `domains` LEFT JOIN `clients` as clients1 ON `clients1`.`id`=`domains`.`client` LEFT JOIN `registrars` as registrars1 ON `registrars1`.`id`=`domains`.`registrar` LEFT JOIN `dns_managers` as dns_managers1 ON `dns_managers1`.`id`=`domains`.`dns_manager` LEFT JOIN `mail_providers` as mail_providers1 ON `mail_providers1`.`id`=`domains`.`mail_provider` ORDER BY 2',
				'server' => 'SELECT `servers`.`id`, `servers`.`hostname` FROM `servers` LEFT JOIN `clients` as clients1 ON `clients1`.`id`=`servers`.`client` LEFT JOIN `hosting_providers` as hosting_providers1 ON `hosting_providers1`.`id`=`servers`.`hosted_at` LEFT JOIN `data_centers` as data_centers1 ON `data_centers1`.`id`=`servers`.`data_center` LEFT JOIN `operating_systems` as operating_systems1 ON `operating_systems1`.`id`=`servers`.`os` ORDER BY 2',
			],
			'registrars' => [
			],
			'dns_managers' => [
			...
			'subdomain_events' => [
				'subdomain' => 'SELECT `subdomains`.`id`, IF(CHAR_LENGTH(`subdomains`.`subdomain`) || CHAR_LENGTH(`subdomains`.`domain`), CONCAT_WS(\'\', `subdomains`.`subdomain`, \'.\', IF(    CHAR_LENGTH(`domains1`.`domain_name`), CONCAT_WS(\'\',   `domains1`.`domain_name`), \'\')), \'\') FROM `subdomains` LEFT JOIN `domains` as domains1 ON `domains1`.`domain_name`=`subdomains`.`domain` LEFT JOIN `servers` as servers1 ON `servers1`.`id`=`subdomains`.`server` ORDER BY 2',
			],
			'client_events' => [
				'client' => 'SELECT `clients`.`id`, `clients`.`client` FROM `clients` ORDER BY 2',
			],
