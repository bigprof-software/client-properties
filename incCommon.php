<?php
	...
	*/
	...
	function get_table_groups($skip_authentication = false) {
		...
		$all_groups = ['Operations', 'Providers', 'Events'];
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			'servers' => "`servers`.`id` as 'id', `servers`.`ip_address` as 'ip_address', IF(    CHAR_LENGTH(`clients1`.`client`), CONCAT_WS('',   `clients1`.`client`), '') as 'client', IF(    CHAR_LENGTH(`hosting_providers1`.`hosting_provider`), CONCAT_WS('',   `hosting_providers1`.`hosting_provider`), '') as 'hosted_at', IF(    CHAR_LENGTH(`data_centers1`.`data_center`), CONCAT_WS('',   `data_centers1`.`data_center`), '') as 'data_center', `servers`.`hostname` as 'hostname', `servers`.`ram` as 'ram', `servers`.`storage` as 'storage', IF(    CHAR_LENGTH(`operating_systems1`.`operating_system`), CONCAT_WS('',   `operating_systems1`.`operating_system`), '') as 'os', `servers`.`comments` as 'comments'",
			'operating_systems' => "`operating_systems`.`id` as 'id', `operating_systems`.`operating_system` as 'operating_system', `operating_systems`.`comments` as 'comments'",
			'hosting_providers' => "`hosting_providers`.`id` as 'id', `hosting_providers`.`hosting_provider` as 'hosting_provider', `hosting_providers`.`comments` as 'comments'",
			'data_centers' => "`data_centers`.`id` as 'id', IF(    CHAR_LENGTH(`hosting_providers1`.`hosting_provider`), CONCAT_WS('',   `hosting_providers1`.`hosting_provider`), '') as 'hosting_provider', `data_centers`.`data_center` as 'data_center', `data_centers`.`comments` as 'comments'",
			'domains' => "`domains`.`domain_name` as 'domain_name', `domains`.`expired` as 'expired', IF(    CHAR_LENGTH(`clients1`.`client`), CONCAT_WS('',   `clients1`.`client`), '') as 'client', IF(    CHAR_LENGTH(`registrars1`.`registrar`), CONCAT_WS('',   `registrars1`.`registrar`), '') as 'registrar', IF(    CHAR_LENGTH(`dns_managers1`.`dns_manager`), CONCAT_WS('',   `dns_managers1`.`dns_manager`), '') as 'dns_manager', IF(    CHAR_LENGTH(`mail_providers1`.`mail_provider`), CONCAT_WS('',   `mail_providers1`.`mail_provider`), '') as 'mail_provider', if(`domains`.`registration_date`,date_format(`domains`.`registration_date`,'%d/%m/%Y'),'') as 'registration_date', if(`domains`.`next_renewal_date`,date_format(`domains`.`next_renewal_date`,'%d/%m/%Y'),'') as 'next_renewal_date', `domains`.`comments` as 'comments'",
			'subdomains' => "`subdomains`.`id` as 'id', `subdomains`.`subdomain` as 'subdomain', IF(    CHAR_LENGTH(`domains1`.`domain_name`), CONCAT_WS('',   `domains1`.`domain_name`), '') as 'domain', IF(    CHAR_LENGTH(`servers1`.`hostname`), CONCAT_WS('',   `servers1`.`hostname`), '') as 'server', `subdomains`.`comments` as 'comments'",
			'registrars' => "`registrars`.`id` as 'id', `registrars`.`registrar` as 'registrar', `registrars`.`comments` as 'comments'",
			'dns_managers' => "`dns_managers`.`id` as 'id', `dns_managers`.`dns_manager` as 'dns_manager', `dns_managers`.`comments` as 'comments'",
			'mail_providers' => "`mail_providers`.`id` as 'id', `mail_providers`.`mail_provider` as 'mail_provider', `mail_providers`.`comments` as 'comments'",
			'clients' => "`clients`.`id` as 'id', `clients`.`client` as 'client', `clients`.`comments` as 'comments'",
			'client_contacts' => "`client_contacts`.`id` as 'id', IF(    CHAR_LENGTH(`clients1`.`client`), CONCAT_WS('',   `clients1`.`client`), '') as 'client', `client_contacts`.`first_name` as 'first_name', `client_contacts`.`last_name` as 'last_name', `client_contacts`.`title` as 'title', `client_contacts`.`mobile` as 'mobile', `client_contacts`.`work_phone` as 'work_phone', `client_contacts`.`email` as 'email', `client_contacts`.`comments` as 'comments'",
			'server_events' => "`server_events`.`id` as 'id', if(`server_events`.`datetime`,date_format(`server_events`.`datetime`,'%d/%m/%Y %h:%i %p'),'') as 'datetime', IF(    CHAR_LENGTH(`servers1`.`hostname`), CONCAT_WS('',   `servers1`.`hostname`), '') as 'server', `server_events`.`event` as 'event', `server_events`.`details` as 'details'",
			'domain_events' => "`domain_events`.`id` as 'id', if(`domain_events`.`datetime`,date_format(`domain_events`.`datetime`,'%d/%m/%Y %h:%i %p'),'') as 'datetime', IF(    CHAR_LENGTH(`domains1`.`domain_name`), CONCAT_WS('',   `domains1`.`domain_name`), '') as 'domain', `domain_events`.`event` as 'event', `domain_events`.`details` as 'details'",
			'subdomain_events' => "`subdomain_events`.`id` as 'id', if(`subdomain_events`.`datetime`,date_format(`subdomain_events`.`datetime`,'%d/%m/%Y %h:%i %p'),'') as 'datetime', IF(    CHAR_LENGTH(`subdomains1`.`subdomain`) || CHAR_LENGTH(`domains1`.`domain_name`), CONCAT_WS('',   `subdomains1`.`subdomain`, '.', `domains1`.`domain_name`), '') as 'subdomain', `subdomain_events`.`event` as 'event', `subdomain_events`.`details` as 'details'",
			'client_events' => "`client_events`.`id` as 'id', if(`client_events`.`datetime`,date_format(`client_events`.`datetime`,'%d/%m/%Y %h:%i %p'),'') as 'datetime', IF(    CHAR_LENGTH(`clients1`.`client`), CONCAT_WS('',   `clients1`.`client`), '') as 'client', `client_events`.`event` as 'event', `client_events`.`details` as 'details'",
		];
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			'servers' => "`servers` LEFT JOIN `clients` as clients1 ON `clients1`.`id`=`servers`.`client` LEFT JOIN `hosting_providers` as hosting_providers1 ON `hosting_providers1`.`id`=`servers`.`hosted_at` LEFT JOIN `data_centers` as data_centers1 ON `data_centers1`.`id`=`servers`.`data_center` LEFT JOIN `operating_systems` as operating_systems1 ON `operating_systems1`.`id`=`servers`.`os` ",
			'operating_systems' => "`operating_systems` ",
			'hosting_providers' => "`hosting_providers` ",
			'data_centers' => "`data_centers` LEFT JOIN `hosting_providers` as hosting_providers1 ON `hosting_providers1`.`id`=`data_centers`.`hosting_provider` ",
			'domains' => "`domains` LEFT JOIN `clients` as clients1 ON `clients1`.`id`=`domains`.`client` LEFT JOIN `registrars` as registrars1 ON `registrars1`.`id`=`domains`.`registrar` LEFT JOIN `dns_managers` as dns_managers1 ON `dns_managers1`.`id`=`domains`.`dns_manager` LEFT JOIN `mail_providers` as mail_providers1 ON `mail_providers1`.`id`=`domains`.`mail_provider` ",
			'subdomains' => "`subdomains` LEFT JOIN `domains` as domains1 ON `domains1`.`domain_name`=`subdomains`.`domain` LEFT JOIN `servers` as servers1 ON `servers1`.`id`=`subdomains`.`server` ",
			'registrars' => "`registrars` ",
			'dns_managers' => "`dns_managers` ",
			'mail_providers' => "`mail_providers` ",
			'clients' => "`clients` ",
			'client_contacts' => "`client_contacts` LEFT JOIN `clients` as clients1 ON `clients1`.`id`=`client_contacts`.`client` ",
			'server_events' => "`server_events` LEFT JOIN `servers` as servers1 ON `servers1`.`id`=`server_events`.`server` ",
			'domain_events' => "`domain_events` LEFT JOIN `domains` as domains1 ON `domains1`.`domain_name`=`domain_events`.`domain` ",
			'subdomain_events' => "`subdomain_events` LEFT JOIN `subdomains` as subdomains1 ON `subdomains1`.`id`=`subdomain_events`.`subdomain` LEFT JOIN `domains` as domains1 ON `domains1`.`domain_name`=`subdomains1`.`domain` ",
			'client_events' => "`client_events` LEFT JOIN `clients` as clients1 ON `clients1`.`id`=`client_events`.`client` ",
		];
